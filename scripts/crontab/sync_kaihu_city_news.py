...
from www.kaihu.models import News, Article, City
...
def sync_kaihu_news():
    ...
    for city in City.objects.filter(is_show=True, location_type=2):
        url = u"http://weixin.sogou.com/weixin?query=%s证券营业部&type=1&ie=utf8&page=%s" % (city.get_city_name_for_seo(), 1)
        resp = requests.get(url, headers=headers, timeout=30)
        text = resp.text
        # print text.encode("utf8")

        jq = pq(text)
        news_hrefs = jq('.txt-box .sp-txt a')
        news_count = 0
        break_flag = False
        ...
        for href in news_hrefs:
            if href is not None:
                ...
                department_count += 1
                if not (News.objects.filter(from_url=href) or News.objects.filter(title=title) or Article.objects.filter(from_url=href) or Article.objects.filter(title=title)):
                    Article.objects.create(title=title, content=content, city_id=city.id, from_url=href, create_time=create_time)
                else:
                    print title.encode("utf8")
                    continue
