...
def crop_img(request):
    ...
    if all((_verfiy_int(x, [0, 300]), _verfiy_int(y, [0, 300]), _verfiy_int(w, [25, 300]), _verfiy_int(h, [25, 300]))):
        ...
        flag, img_name_600 = qiniu_client.upload_img(urllib.urlopen(request.user.get_avatar_600()))
        if flag:
            # 拼接给七牛的参数
            post_url = '%s/%s?imageMogr2/crop/!%sx%sa%sa%s' % (settings.IMG0_DOMAIN, img_name_600, w, h, x, y)
            ...
            if flag:
                ...
                # 删除多余的两张图片节省空间
                qiniu_client.batch_delete([img_name_original, img_name_600])
            else:
